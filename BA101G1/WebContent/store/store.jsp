			...
			<div id="main">
				...
				<FORM METHOD="post"
					...
					<b>選擇地區 :</b> <select size="1" name="store_zone">
						...
						<option value="臺北市"
							<c:if test="${store_zone.equals('臺北市') }"> selected</c:if>>臺北市
						
						<option value="新北市"
							<c:if test="${store_zone.equals('新北市') }"> selected</c:if>>新北市
						
						<option value="桃園市"
							<c:if test="${store_zone.equals('桃園市') }"> selected</c:if>>桃園市
						
						<option value="新竹市"
							<c:if test="${store_zone.equals('新竹市') }"> selected</c:if>>新竹市
						
						<option value="新竹縣"
							<c:if test="${store_zone.equals('新竹縣') }"> selected</c:if>>新竹縣
						
						<option value="苗栗縣"
							<c:if test="${store_zone.equals('苗栗縣') }"> selected</c:if>>苗栗縣
						
						<option value="臺中市"
							<c:if test="${store_zone.equals('臺中市') }"> selected</c:if>>臺中市
						
						<option value="彰化縣"
							<c:if test="${store_zone.equals('彰化縣') }"> selected</c:if>>彰化縣
						
						<option value="南投縣"
							<c:if test="${store_zone.equals('南投縣') }"> selected</c:if>>南投縣
						
						<option value="雲林縣"
							<c:if test="${store_zone.equals('雲林縣') }"> selected</c:if>>雲林縣
						
						<option value="嘉義市"
							<c:if test="${store_zone.equals('嘉義市') }"> selected</c:if>>嘉義市
						
						<option value="嘉義縣"
							<c:if test="${store_zone.equals('嘉義縣') }"> selected</c:if>>嘉義縣
						
						<option value="臺南市"
							<c:if test="${store_zone.equals('臺南市') }"> selected</c:if>>臺南市
						
						<option value="高雄市"
							<c:if test="${store_zone.equals('高雄市') }"> selected</c:if>>高雄市
						
						<option value="屏東縣"
							<c:if test="${store_zone.equals('屏東縣') }"> selected</c:if>>屏東縣
						
						<option value="臺東縣"
							<c:if test="${store_zone.equals('臺東縣') }"> selected</c:if>>臺東縣
						
						<option value="花蓮縣"
							<c:if test="${store_zone.equals('花蓮縣') }"> selected</c:if>>花蓮縣
						
						<option value="宜蘭縣"
							<c:if test="${store_zone.equals('宜蘭縣') }"> selected</c:if>>宜蘭縣

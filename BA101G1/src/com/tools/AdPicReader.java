...
public class AdPicReader extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		String name = req.getParameter("ad_id"); // 後
		String name2 = new String(name.getBytes("ISO-8859-1"), "big5"); // 再

		try {
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery(
				"SELECT ad_image  FROM ad WHERE ad_id = '" + name2 +"'"); //等號後的字串(name2)有分大小寫
			if (rs.next()) {
				BufferedInputStream in = new BufferedInputStream(rs.getBinaryStream("ad_image"));
